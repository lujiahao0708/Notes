...
//        mLoader.execute();
        ...
        AsyncTaskFactory asyncFactory = AsyncTaskFactory.getInstance();
        asyncFactory.addSyncTask(new AsyncTaskFactory.IProgressCallback() {
            // 耗时操作，该方法是在syncTask的线程中调用。
            @Override
            public void progressCallback(AsyncTaskFactory.AsyncResult result) {
                // 下载完数据后的操作，如，写到文件中，设置对象的变量.....
                Log.e("progressCallback",result._url+"    "+result._byteBuffer.toString());
            }

        },new AsyncTaskFactory.IResultCallback() {
            // 该方法是被UI线程调用
            @Override
            public void resultCallback(AsyncTaskFactory.AsyncResult result) {
                // 把该数据设置到对应的UI上，如，通过byte[] 创建 bitmap
                Bitmap bitmap = BitmapFactory.decodeByteArray(result._byteBuffer,0,result._byteBuffer.length);
                ivImage.setImageBitmap(bitmap);
                Log.e("resultCallback",result._url+"    "+result._byteBuffer.toString());
            }
        }, url);
    }
